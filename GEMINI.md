...
### 2026-01-23: Límite de 1000 registros en Supabase SELECT
- **Error**: La importación de alquileres fallaba silenciosamente omitiendo miles de registros porque `supabase.from().select()` tiene un límite por defecto de 1000. Al mapear 4000+ clientes, la mayoría no se encontraban.
- **Fix**: Implementar bucles `while` con `.range(offset, offset + 999)` para cargar TODOS los registros de mapeo (clientes, artículos, alquileres existentes).
- **Aplicar en**: Cualquier fetch masivo de tablas maestras para mapeos en memoria.

### 2026-01-23: Conectividad Vercel vs IPs Locales (Pendiente)
- **Error**: Vercel no puede conectar a MSSQL usando IPs internas (192.168.x.x).
